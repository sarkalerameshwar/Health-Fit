...
const Inquiries = () => {
 ...
 const fetchInquiries = async () => {
  try {
    ...
    setInquiries(data.success ? data.inquiries : []); // <-- fix here
    setError(null);
  } catch (error) {
  ...
  const updateInquiryStatus = async (inquiryId, newStatus) => {
    try {
      const response = await fetch(`http://localhost:5000/api/inquiries/update-status/${inquiryId}`, {
        method: 'PATCH',
        headers: {
...
User: ${inquiry.userId?.name || 'N/A'} (${inquiry.email})
...
Status: ${inquiry.status}`);
  ...
  const filteredInquiries = inquiries.filter(inquiry => {
    ...
    const matchesSearch = searchTerm === '' ||
      ...
      (inquiry.userId?.name && inquiry.userId.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const columns = [
    ...
    { 
      ...
      accessor: 'subject',
      render: (item) => (
        <div className="truncate max-w-xs">{item.subject}</div>
      )
    },
    { 
      ...
      render: (item) => (
        <select
          ...
          className={`px-2 py-1 rounded text-xs font-medium border-none focus:ring-2 focus:ring-opacity-50 ${
            item.status === 'open' ? 'bg-blue-100 text-blue-800 focus:ring-blue-500' :
            item.status === 'in progress' ? 'bg-yellow-100 text-yellow-800 focus:ring-yellow-500' :
    ...
    <div>
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-dark">Customer Inquiries</h2>
          <button 
